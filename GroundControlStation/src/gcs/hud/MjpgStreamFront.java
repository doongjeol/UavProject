...
public class MjpgStreamFront {
    private Canvas canvas;
    private GraphicsContext gc;
    private MqttClient mqttClient;

    public MjpgStreamFront(Canvas canvas) throws Exception {
        this.canvas = canvas;
                    ...
                    try {
                        ...
                        mqttClient.setCallback(new MqttCallback() {
                            ...
                            public void messageArrived(String topic, MqttMessage message) throws Exception {
            					...
            					mqttClient.publish(Network.uavCameraFrontSubTopic, nextMessage);
                        ...
                        mqttClient.subscribe(Network.uavCameraFrontPubTopic);
                        connected = true;
                        mqttClient.publish(Network.uavCameraFrontSubTopic, new byte[0], 0, true); // MQTT Broker의 영상버퍼 제거
                        MqttMessage nextMessage = new MqttMessage("next".getBytes()); // next Message 제작
    					mqttClient.publish(Network.uavCameraFrontSubTopic, nextMessage.getPayload(), 0, true); // next Message 발송
                    } catch(Exception e) {
                        try {
                            MjpgStreamFront.this.stop();
                        } catch(Exception e2) {}
                    }
                }
            }
